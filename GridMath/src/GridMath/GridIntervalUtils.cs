...
namespace GridMath
{
    ...
    /// </summary>
    public static class GridIntervalUtils
    {
        public static IntegerInterval[] Separate(IntegerInterval i1, IntegerInterval i2)
        {
            var depth = i1.Depth(i2);
            if (depth == 0) return new[] {i1, i2};
            var t1 = LineGridTransforms.RealToGrid(depth / 2.0);
            var t2 = depth % 2 == 0 ? -t1 : LineGridTransforms.RealToGrid(-(depth + 1) / 2.0);
        ...
        ///     input array
        /// </returns>
        public static List<List<int>> FindOverlappingIntervals(IntegerInterval[] intervals)
        {
            ...
            while (endsIndex < sortedEnds.Length)
            {
                ...
                else
                {
                    ...
                    {
                        if (listOfOverlappingLists.All(list => list.Intersect(currentOverlapList).Count() != currentOverlapList.Count))
                        {
        ...
        public static bool SomeOverlap(IntegerInterval[] intervals)
        {
            return FindOverlappingIntervals(intervals).Count > 0;
        }

        public static int FindCenterOfMass(IntegerInterval[] intervals)
        {
